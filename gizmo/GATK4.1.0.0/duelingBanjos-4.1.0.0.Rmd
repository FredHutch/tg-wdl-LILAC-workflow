...
# Load Packages
```{r}
library(tidyverse); library(tgR); library(fh.wdlR); library(aws.s3)
```
...
annotations <- tgrAnnotate(DAG = "paguirigana", harmonizedOnly = FALSE)
...
```{r}
batchFileName <- paste0("cromwell-manifests/",format(Sys.Date(), "%Y-%m-%d-"), "duel-4.1.0.0-test1.tsv")
...
thisManifest <- "cromwell-manifests/2020-04-17-duel-4.1.0.0-test1.tsv"
```
...
setCromwellURL(FredHutchId = "apaguiri", jobId = "47054223", port = "8000")
...
thisJob <- cromwellSubmitBatch(WDL = "duelingBanjos_GATK4.1.0.0.wdl",
                    ...
                    Labels = data.frame("workflowType" = "first test after 1.0"))
thisOne <- thisJob$id; thisOne
```

```{r}
thisManifest <- "cromwell-manifests/2020-04-17-duel-4.1.0.0-test1.tsv"
thisOne <- "b926e171-6550-433d-913a-d892d053fd6b"
```
...
Workflow is:  tg-wdl-copyNTag
Download the current master branch copy and unzip the file. 
```{r}
workflowDir <- paste0("./Workflows/validatetwoworkflows/copy-", thisOne, "/")
...
workflowFiles <-  "./Workflows/validatetwoworkflows/copy-b926e171-6550-433d-913a-d892d053fd6b//tg-wdl-copyNTag-master/gizmo"   
```
