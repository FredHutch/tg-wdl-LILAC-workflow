...
manifest <- matched
manifest <- manifest[1,]
manifest <- matched %>% filter(molecular_id %in% c("M00001494", "M00001491", "M00001221"))
```
...
batchFileName <- paste0("cromwell-manifests/", repoName, "/", format(Sys.Date(), "%Y-%m-%d-"), "LILAC-EGFR-mantaTestTrio.tsv")
...
Sys.setenv("CROMWELLURL" = "http://gizmo....")
...
```{r}
setwd("~/github/workflows/tg-wdl-LILAC-workflow/GATK4.1.4.0/manta")
thisJob <- cromwellSubmitBatch(WDL = "pileon_GATK4.1.4.0_manta.wdl",
                    Params = "pileon_parameters_hg19-homosapiens.json",
                    Batch = "pileon_batch.json"   ,
                    Options = "continueWhile-callcachingOn.json",
                    Labels = data.frame("workflowType" = "testingEGFRindel-Trio"))
thisOne <- thisJob$id; thisOne
```
...
thisOne <- "ae66870f-a454-4992-9e9b-3a89d5bac232" # Manta trio test
```
